...
raw_obs <- read.csv(file.path(PATH_DATA, "all_operations_on_fobs_observe_v9_fr_2005-2023.csv"))
Ob7 <- prep.obs(Ob7 = raw_obs)

Ob7 <- Ob7[Ob7$year>2013,]
Ob7 <- Ob7[Ob7$year<2023,]
Ob7_NLOG = Ob7 %>% filter(obj_conv=="NLOG")
Ob7_NLOG$Zone <- as.factor(ifelse(Ob7_NLOG$latitude<(-10) & Ob7_NLOG$longitude <= 50,"MOZ","WIO"))
...
```{r}
ggsave(file=file.path(PATH_FIG, "Fig1_Study Area.png"), fig1,width = 10, height = 9, units = "cm")
ggsave(file=file.path(PATH_FIG, "Fig1.eps"), fig1,width = 10, height = 9, units = "cm")
```
...
f3.ssci.moz <- ggplot()+
  ...
  annotate(geom="text", x=0.445, y=8.75, label="P = 0.588",
...
f3.ssci.north <- ggplot()+
  ...
  ggtitle("WIO")+ #\U2265 is the code for the >= symbol
  theme(text = element_text(size = 20))+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(axis.text.x = element_text(size = 20))+
  theme(axis.text.y = element_text(size = 20))+
  theme(legend.text = element_text(size=20))+
  theme(legend.title = element_text(size=20))+
  annotate(geom="text", x=0.62, y=4.3, label="P = 0.012",
...
f3.sla.moz <- ggplot()+
  ...
  theme(legend.text = element_text(size=20))+
  theme(legend.title = element_text(size=20))+
  annotate(geom="text", x=0.14, y=13, label="P = 0.019",
              color="black", fontface="bold.italic", size=6)
```
...
f3.sla.north <- ggplot()+
  ...
  annotate(geom="text", x=0.28, y=5, label="P = 0.844",
              color="black", fontface="bold.italic", size=6)
```
...
f3.fsle.moz <- ggplot()+
  ...
  annotate(geom="text", x=-0.037, y=15, label="P = 0.857",
              color="black", fontface="bold.italic", size=6)
```
...
f3.fsle.north <- ggplot()+
  ...
  annotate(geom="text", x=0, y=21, label="P = 0.155",
              color="black", fontface="bold.italic", size=6)
```
...
f3.mn.moz <- ggplot()+
  ...
  annotate(geom="text", x=1.2, y=4.5, label="P = 0.364",
              color="black", fontface="bold.italic", size=6)
```
...
f3.mn.north <- ggplot()+
  ...
  annotate(geom="text", x=1.04, y=2.8, label="P = 0.153",
              color="black", fontface="bold.italic", size=6)
```
...
f3.chla.moz <- ggplot()+
  ...
  annotate(geom="text", x=0.93, y=9.1, label="P = 0.422",
              color="black", fontface="bold.italic", size=6)
```
...
f3.chla.north <- ggplot()+
  ...
  annotate(geom="text", x=0.69, y=6.5, label="P = 0.020",
              color="black", fontface="bold.italic", size=6)
```
...
p1 <- plot.gam.prediction(data = NLOG_VE_sup_zero_North,
                    my_gam = GAM_North2,
                    vars = c("chlacr","slacr","SSCIcr","FSLEcr"),
...
fig5 <- ggarrange(p1,
                  ...
                  label.y = 0.95,
          ncol = 1, nrow = 2)
ggsave(file=file.path(PATH_FIG, "Fig5_gam_predict.png"), fig5, width = 8, height = 15, units = "cm")
ggsave(file=file.path(PATH_FIG, "Fig5.eps"), fig5, width = 8, height = 15, units = "cm")
```
...
#Before
f.bef <- ggplot()+
  geom_histogram(data = NLOG_VE_sup_zero, aes(x = NLOG_stand),
                 binwidth = 0.1)+
...
B1.ssci.north <- ggplot()+
  geom_histogram(stat = 'bin', aes(x=SSCImean, y = ..density.., fill = "Data used"),
                 binwidth = 0.02, bins = 30, size = 1,
                 data = df_eff[df_eff$threshold=="Fisheries" & df_eff$Zone=="WIO",], alpha=0.5)+
  geom_histogram(stat = 'bin', aes(x=SSCImean, y = ..density.., fill = "Random data"),
                 binwidth = 0.02, bins = 30, size = 1,
